...
bool msg_exception(char *message, bool error_exit){
...
bool lock_file(bool error_exit) {
    ...
    if (write(fd_lock_file, pid, strlen(pid)) != strlen(pid) || close(fd_lock_file) == -1) 
        return msg_exception(LOCK_FILE_ERROR, error_exit);
    return true;
}


bool unlock_file(bool error_exit) {
    ...
    if (
        ...
        || remove(flock_name) == -1)
        return msg_exception(UNLOCK_FILE_ERROR, error_exit);
...
int main(int argc, char** argv) {
    ...
    count_lock_unlock(error_exit, sleep_time);
